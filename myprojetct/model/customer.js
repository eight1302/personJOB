...
var vue = new Vue({
  ...
  methods:{
    ...
    //处理列表展示省
    addreprovince : function(row){
      if(row.province!=''){
        for(var i=0;i<this.province.length;i++){
          if(this.province[i].id == row.province){
            return this.province[i].value;
          }
        }
      }
    },
     ...
     //处理列表展示市
     addrecity : function(row){
      if(row.city!=''){
        for(var i=0;i<this.city.length;i++){
          if(this.city[i].id == row.city){
            return this.city[i].value;
          }
        }
      }
    },
    ...
    //处理列表展示县
    addredistrict : function(row){
      if(row.district!=''){
        for(var i=0;i<this.blockall.length;i++){
          if(this.blockall[i].id == row.district){
            return this.blockall[i].value;
          }
        }
      }
    },
    ...
    getData:function(data,currentPage,pageSize){
      ...
      if(data.user!='' || data.name!=''){
        if(data.user!='' && data.name!=''){
          this.db.transaction(function (tx) {
            tx.executeSql('SELECT * FROM clientdata WHERE user="'+data.user+'" AND name="'+data.name+'" order by id desc LIMIT '+star+','+end, [], function (tx, results) {
          ...
          if(data.user!='' && data.name==''){
            this.db.transaction(function (tx) {
              tx.executeSql('SELECT * FROM clientdata WHERE user="'+data.user+'" order by id desc LIMIT '+star+','+end, [], function (tx, results) {
                vue.tableData =results.rows;
                if(results.rows.length<pageSize){
                  vue.pageSize = 10;
                }else{
          ...
          if(data.user=='' && data.name!=''){
            this.db.transaction(function (tx) {
              tx.executeSql('SELECT * FROM clientdata WHERE name="'+data.name+'" order by id desc LIMIT '+star+','+end, [], function (tx, results) {
                vue.tableData =results.rows;
                if(results.rows.length<pageSize){
                  vue.pageSize = 10;
                }else{
        ...
        this.db.transaction(function (tx) {
          tx.executeSql('SELECT * FROM clientdata order by id desc LIMIT '+star+','+end, [], function (tx, results) {
            vue.tableData =results.rows;
            if(results.rows.length<pageSize){
              vue.pageSize = 10;
            }else{
    ...
    closeDialog : function(){
     ...
     hasCancel:function(id,name){
      this.$confirm('您确定要删除次数据吗?', {
        ...
        this.db.transaction(function (tx) {
          tx.executeSql('DELETE FROM clientdata WHERE id='+id);
          tx.executeSql('INSERT INTO log (username,name,state,time) VALUES (?,?,?,?)', [localStorage.getItem("user"),"删除客户名称为:"+name,"删除成功",Date.parse(new Date())]);
          vue.getData(vue.select,1,10);
        });
      }).catch(() => {
        this.$message({
    ...
    getOnedata : function(id){
      ...
      this.db.transaction(function (tx) {
        tx.executeSql('SELECT * FROM clientdata where id='+id, [], function (tx, results) {
    ...
    enterpriseedit : function(){
     ...
     showdata : function(){
      ...
      this.client.contacts_name = this.clientDetail[0].contactsname;
      this.client.contacts_tel = this.clientDetail[0].contactstel; 
      this.client.importance = this.clientDetail[0].importance; 
      this.client.province = this.clientDetail[0].province; 
      this.client.city = this.clientDetail[0].city; 
      this.client.district = this.clientDetail[0].district; 
      this.client.address = this.clientDetail[0].address;
    },
  }
});
